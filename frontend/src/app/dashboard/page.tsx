...
export default function Dashboard() {
  ...
  const openBook = async (book: any) => {
    try { const saved = await api.post(`/books/save?ol_id=${book.open_library_id}`); router.push(`/book/${saved.data.id}`) }
    catch (e) { console.error(e) }
  }
  ...
  const submitReview = async () => {
    try {
      ...
      setShowReviewForm(false); fetchMyReviews(); alert('Review submitted !')
          ...
          <div>
            ...
            {results.length === 0 && !loading && (
              <div style={{ textAlign: 'center', padding: '80px 0', color: '#4B5563' }}>
                ...
                <p style={{ fontSize: '16px' }}>Search for any book to get started</p>
              </div>
            )}
          </div>
        )}
